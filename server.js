...
// AI SCAN — uses Anthropic web_search to find Nordic logistics news
app.post('/api/scan', async (req, res) => {
  const apiKey = req.headers['x-anthropic-key'];
  if (!apiKey) return res.status(400).json({ error: 'x-anthropic-key header required' });

  const { tenants = [], prospects = [] } = req.body;

  const queries = [
    'Nordic logistics company funding round contract win market entry capacity expansion hiring warehouse Sweden Denmark Finland 2025 2026',
  ];

  const SCAN_COMPANIES = [
    'DSV', 'PostNord', 'Bring', 'DB Schenker', 'DHL Sweden', 'DHL Freight',
    'DHL Express', 'Maersk', 'DFDS', 'Stena Line', 'UPS Sweden', 'FedEx Sweden',
    'NTG Nordic Transport Group', 'Scan Global Logistics', 'GLS Denmark',
    'dao Denmark', 'Jetpak', 'Innight', 'Ntex', 'Freja Transport',
    'Dachser Nordic', 'Geodis Nordic', 'CEVA Logistics',
    'XPO Logistics Nordic', 'PostNord Danmark', 'Schenker Finland',
    'Zalando', 'Amazon Nordic', 'Kuehne+Nagel Nordic'
  ];
  const companyList = [...(tenants||[]).slice(0,5), ...(prospects||[]).slice(0,5), ...SCAN_COMPANIES].slice(0,30).join(', ');
    ...
    try {
      const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          system: `You are a Nordic logistics real estate analyst hunting PRE-LEASE signals for these priority companies: ${companyList}. Search for: funding rounds, major contract wins, market entry announcements, capacity constraints, large hiring surges, M&A activity — signals that precede a warehouse lease decision by 6-18 months. Output ONLY a valid JSON array, start with [ end with ]. Each item: {"title":"...","description":"1-2 sentences explaining why this is a space demand signal","link":"...","date":"YYYY-MM-DD or null","company":"...","country":"sweden|denmark|finland","region":"city"}. Max 8 items. Raw JSON only — no prose.`,
          messages: [{ role: 'user', content: `Search for recent 2025-2026 Nordic logistics news: funding rounds, contract wins, market entry, capacity constraints, hiring surges for companies like: ${companyList}` }]
        })
      });

      const data = await resp.json();
      console.log('Anthropic response type:', data.type, 'stop_reason:', data.stop_reason, 'content blocks:', data.content?.length);
      if (data.error) { console.warn('Anthropic error:', JSON.stringify(data.error)); continue; }

      // With web_search tool, collect all text blocks
      const textBlocks = (data.content || []).filter(b => b.type === 'text').map(b => b.text).join('');
      console.log('Text content length:', textBlocks.length, 'preview:', textBlocks.slice(0,200));
      if (!textBlocks) continue;

      const match = textBlocks.match(/\[[\s\S]*\]/);
      if (!match) { 
        // Try to find partial array
        const partial = textBlocks.indexOf('[');
        if (partial === -1) { console.warn('No JSON array found in response'); continue; }
        console.warn('No complete JSON array — response likely cut off at max_tokens');
        continue;
      }

      let items;
      try { items = JSON.parse(match[0]); } catch(e) { console.warn('JSON parse failed:', e.message, match[0].slice(0,100)); continue; }
  ...
  console.log(`AI scan: ${allArticles.length} articles found`);

  // Classify
  const signals = allArticles.map((a, i) => classifyRuleBased(a, i, tenants, prospects)).filter(Boolean);
  ...
  res.json({ signals, meta: { total: signals.length, articles: allArticles.length } });
});
